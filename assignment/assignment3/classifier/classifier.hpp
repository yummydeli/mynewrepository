...
namespace sp
{
    ...
    public:
        ...
        inline int forward(cv::Mat mat) // noexcept
        {
            ...
            int id = 0;
            for(; id < good_templates.size(); ++id)//先匹配正模板
            {
                ...
                if(max_belief < now_belief)
                {
                    ...
                    match_id = id;
                std::cout << match_id << "nm --- " << max_belief << '\n';
                }
            }
            ...
            for(int i = 0; i < bad_templates.size(); ++id, ++i)//后匹配负模板
            {
        ...
        //若负样本在与正模板匹配中最高得分均小于1000则它就是负样本，且分数应该都为负数，都小于1000,则此次是返回的id为good_templates.size，计数时则应取比较值的反。
    ...
    private:
        inline container_t make_templates_(const std::string& where)//创建模板的原始函数
        {
            container_t ret;
        ...
        inline int  get_threshold_(cv::Mat& mat)//取一张图片中像素值出现次数为前25%的像素值作为阈值
        {
        ...
        inline int compare_(cv::Mat& tem,cv::Mat& true_sample)
        {
            ...
            for (uint32_t i = 0; i < iter_rows; ++i)
            {
                ...
                {
                    bool l = *lhs++, r = *rhs++;
                    if(l==0 && r==0)//若两个像素点都为0则不计分，即排除黑色区域
                        continue;
                    if(l && r==1)//若两个像素点都为1则加三分
                    {
                        same_cnt+=3;
                    }
                    else//若两个像素中不全为1则减2分
